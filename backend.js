(() => {
  ...
  async function submitReview(month, review) {
    ...
    if (mode === 'cloud') {
      const result = await client.rpc('submit_monthly_review', {
        p_month: month,
    ...
    db.reviews.push({
      ...
      renew: review.renew, submitted_at: new Date().toISOString()
    });
    writeDemo(db);
  }
  ...
  async function addPhoto(file) {
    ...
    if (current.length >= 24) throw new Error('Album quota reached');
  ...
  async function exportData() {
    ...
    return { exportedAt: new Date().toISOString(), pair: context.pair, membership: context.membership, visibleReviews: reviews, albumMetadata: photos.map(({ url, blob, ...row }) => row) };
  }
  ...
  window.TsukiBackend = {
    ...
    getPhotos, addPhoto, deletePhoto, updateProfile, rotateInvite, rotateRecovery, exportData, deleteAccount,
